...
// string) and later sampled probabilistically. The 4->1, 3->1, and 2->1
// cases are coloured differently.
...
string select(const string &s)
{
   ...
   if (result.size() == 1) return result[0];
      ...
      return result[size_t(rnd()*result.size())];
}
...
int main(int argc, char* argv[])
{
...

   {
      ...
      for (size_t n = 0; n < 250; ++n)
      {
         ...
         {
            colour = BrightGreen;
            response = select(dictionary4[words]);
         }
         else if (dictionary3.contains({ words[1], words[2], words[3] }))
         {
            colour = BrightYellow;
            response = select(dictionary3[{ words[1], words[2], words[3] }]);
         }
         else if (dictionary2.contains({ words[2], words[3] }))
         {
            colour = BrightBlue;
            response = select(dictionary2[{ words[2], words[3] }]);
         }
         else if (dictionary1.contains(words[3]))
         {
            colour = BrightMagenta;
            response = select(dictionary1[words[3]]);
         }
         else
         {
